...
def check_antisyms_in_cell(Cell, Vacancies, matrices):
    ...
    transformed_vacancies = Vacancies @ MAT.matrices
    mask = np.full(
        len(matrices), True, dtype=bool
    )  # matrixes.shape[0] instaed of len()
...
def get_matrixes(allowed_matrixes):
    return np.array(
        [
            ...
            for mat, mask in zip(
                MAT.all_matrixes,
                np.ma.masked_where(allowed_matrixes, MAT.all_labels).mask,
            )
            if mask
        ]
    )
...
def check_all_cells2(scell, sumVac):
    mylist = []
    labels_in = MAT.labels
    matrixes = MAT.matrices
