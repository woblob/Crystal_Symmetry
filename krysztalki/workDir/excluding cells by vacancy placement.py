...
def func(UVWs):
    return all(checkforINT(makeUVW(UVWs[e1], UVWs[e2])) for e1, e2 in COMB(range(len(UVWs)),2))
...
def funcvar(vacs):
    return all_same(makeUVW, vacs) and \
        not all_same(np.linalg.norm, vacs) and \
        all_same(checkforINT, list(map(makeUVW,vacs))) and \
        func(vacs)
...
def abc_Dict(scell,base,sumVac):
    ...
    for indexes in itrtls_combinations(range(len(scell)),sumVac): 
        ...
        mylist.append([ SYM != [], all_same(makeUVW, vacancies), all_same(np.linalg.norm, vacancies) , all_same(checkforINT, list(map(makeUVW,vacancies))) , func(vacancies) ])
