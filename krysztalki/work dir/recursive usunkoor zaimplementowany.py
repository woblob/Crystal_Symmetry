...
def check_symmetries_in_cell(cell, matrices=mat.all_matrices):
    ...
    transformed_points_sorted = \
        np.array([np.unique(points, axis=0) for points in transformed_points])
    return np.array([np.array_equal(points, cell)
                     for points in transformed_points_sorted])
...
class ReducedCell(NodeMixin):
    def __init__(
        ...
        self.symmetry_mask_cell = \
            check_symmetries_in_cell(self.cell)
        self.symmetry_labels_cell = \
            output_symmetries(self.symmetry_mask_cell)
        self.reduced_cell, self.reduced_cell_indexes = \
            Tree.reduce_cell_by_symmetry2(
                self.cell,
                self.cell_indexes,
                self.symmetry_mask_cell
            )
...
class Tree:
    ...
    def __init__(self, file, size, vacancy_amount=None, vacancy_index=None):
        ...
        self.cell, self.atoms_labels, self.base_type = \
            cPrs.get_super_cell3(self.file, self.size)
        self.cell_indexes = np.arange(self.cell.shape[0])
        self.unchanged_cell_aka_root = \
            ReducedCell(self.cell, self.cell_indexes, self.atoms_labels, None)
    ...
    def _get_matrices_for_specific_point_group(mask_present_symmetries):
        present_operations = np.ma.masked_where(
            mask_present_symmetries,
            mat.all_operations
        ).compressed()

        mask_for_matrices = \
            np.array([np.full((3, 3), val) for val in mask_present_symmetries])

        present_matrices = np.ma.masked_where(
            mask_for_matrices,
            mat.all_matrices
        ).compressed().reshape((-1, 3, 3))
    ...
    def reduce_cell_by_symmetry(cell, symmetries):
        matrices, operations = \
            Tree._get_matrices_for_specific_point_group(symmetries)
        ...
        #            index))]
        reduced_cell, reduced_cell_indexes = \
            zip(*((point, index) for point, index in cell_dict.items()
                  if index))
        reduced_cell, reduced_cell_indexes = \
            np.array(reduced_cell), np.array(reduced_cell_indexes)
    ...
    def make_cell_with_vacancies(cell, index):
        ...
        cell_vac = []
        vac = []
        for nr, p, i, l in enumerate(zip(cell.cell,
                                         cell.cell_indexes,
                                         cell.atoms_labels)):
